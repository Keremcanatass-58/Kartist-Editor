...
- Key switches read in `Program.cs`:
  ...
  - `Ai:*` — selects AI providers (see below). `OpenAI:ApiKey` / `Groq:ApiKey` / `Gemini:ApiKey` / `Pexels:ApiKey` are read out-of-band by the services.
...
4. SignalR hubs are mapped at `/adminHub`, `/notificationHub`, and `/notifHub` (alias). `NotificationHub` maintains a static `UserConnections` dictionary keyed by email (the `Identity.Name` claim). It also handles **live streaming signaling** — see "Live streaming" below.
...
- Turkish schema — core: `Kullanicilar` (users), `Sablonlar` (templates), `SosyalGonderiler` (posts), `SosyalBegeniler` (likes), `SosyalYorumlar` (comments), `Takipciler` (follows), `Hikayeler` (stories), `DirektMesajlar` (DMs), `Bildirimler` (notifications). Gamification: `Rozetler` (badges), `KullaniciRozetleri`, `KullaniciXP`, `GunlukGorevler` (daily quests). Security: `IkiFactorKodlari` (2FA OTPs), `GirisLoglari` (login audit). Other: `Hashtagler`, `CanliYayinlar` (live streams). Keep new identifiers in the same language.
...
- `IAiPromptService` → **Gemini** (`gemini-2.0-flash` via OpenAI-compatible endpoint, now the default) or Groq (`llama-3.3-70b-versatile`) or OpenAI chat. Provider is selected by `Ai:PromptProvider`; the Gemini path sends the API key as a query parameter, not a Bearer header. Config keys: `Gemini:ApiKey`, `Ai:GeminiEndpoint`, `Ai:GeminiModel`.
- `AiImageService` also supports **Pexels** (`Pexels:ApiKey`) for real-photo search results — the prompt service generates `revisedImagePrompt` keywords tuned for Pexels (architecture/nature/landscape only, never literal card/letter searches).
...
### Live streaming
`SocialController` exposes `Live()`, `YayinBaslat()`, `YayinBitir()`, `GetAktifYayinlar()`, and `JoinLiveRoom()`. Broadcasting uses **WebRTC peer connections** signaled through `NotificationHub` (`StartBroadcast`, `JoinStream`, `SendOffer/Answer/IceCandidate`). The hub tracks active broadcasts in static dictionaries `StreamBroadcasters` and `StreamViewers`. Stream metadata is stored in the `CanliYayinlar` table. The frontend (`Views/Social/Live.cshtml`) handles camera + screen-capture PiP, muted autoplay fallback for mobile, and live chat via `SendLiveMessage`/`ReceiveLiveMessage`.

### Duels & Competitions
`Views/Social/Duels.cshtml` and `Views/Social/Competitions.cshtml` are UI-complete with hero stats, category filters, and voting cards. `VoteDuel()` awards XP via `GunlukGorevIlerle("duello")`. These views currently use **mock data from ViewBag** — there is no `Duellolar` table yet. New features in this area should wire up real DB tables in `DatabaseInitializer.cs`.

### XP & gamification
Users accumulate XP (`Kullanicilar.ToplamXP`, `Kullanicilar.Seviye`) tracked in the `KullaniciXP` transaction log. `GunlukGorevler` stores daily quest progress per user. `Rozetler` (badges) are seeded in `DatabaseInitializer` with `XPOdulu` rewards; user awards go into `KullaniciRozetleri`. The leaderboard in `SocialController.Feed()` queries `ToplamXP DESC`.

### Frontend stack
- **CSS**: Tailwind CSS (CDN), custom CSS variables for dark/light theming (`--foreground`, `--background`, `--accent`, etc.), Space Grotesk font.
- **Icons**: Lucide (`data-lucide` attributes, initialized via `lucide.createIcons()`) + FontAwesome 6.5.0.
- **Core JS**: `wwwroot/js/kartist-social.js` (v3.0) provides CSRF token helpers, `escapeHtml` XSS guard, media URL normalization, secure fetch wrapper, toast notifications, Turkish time-ago formatting, avatar HTML generation, and SignalR notification integration. All user-generated content is escaped before `innerHTML`.
- **Layout variants**: `_Layout.cshtml` (marketing/public pages), `_SocialLayout.cshtml` (legacy social sidebar — only used by `Kesf`), `_SocialLayoutModern.cshtml` (current social pages — Feed, Live, Duels, Profil, Messages, etc.). New social views should use `_SocialLayoutModern`.
...
- `docs/` contains sprint reports and the vize project report (Turkish academic submission). `notlar` at repo root holds Sprint 4 security-audit notes (jury-prep internal); read it before changing any of the Sprint 4 hardening commits referenced above.
